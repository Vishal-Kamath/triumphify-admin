...
import { useCategorySales } from "@/lib/sales";
...
const OrderVariationSale: FC<{
  ...
  const { data: sales, isLoading } = useCategorySales(type, startDate, endDate);
      ...
      <table className="border-collapse w-full max-w-full select-none overflow-x-auto border-1 text-sm scrollbar-thin max-lg:block [&>*>*>*]:border-1 [&>*>*]:border-1 [&>*]:border-1">
        ...
        <tbody>
          {sales.map((sale, index) => {
            return (
              <tr key={index + sale.category_id}>
                <td className="min-w-28 w-full break-words p-3">
                  <div className="flex flex-col">
                    <h3>{sale.category_name}</h3>
                    <Link
                      href={`/products/categories/${sale.category_id}`}
...
const OrderVariationSaleBar: FC<{
  ...
  const labels = sales.map((sale) => sale.category_name);
  const datasets = [
    {
