...
import {
  UpdateBannerFormType,
  updateBannerFormSchema,
} from "./update-banner-form-schema";
...
import { invalidateAllBanners, useBanner } from "@/lib/banners";

const UpdateMainBannerForm: FC = () => {
  const { id, type } = useParams() as { type: "main" | "sub"; id: string };
  const router = useRouter();
  const { toast } = useToast();

  const { data: banner } = useBanner("main", id);
  ...
  useEffect(() => {
    if (!banner) return;
    form.setValue("banner_image_desktop", banner.banner_image_desktop);
    form.setValue("banner_image_mobile", banner.banner_image_mobile);
    form.setValue("link", banner.link);
    form.setValue("is_published", banner.is_published);
    setImageBase64Desktop(banner.banner_image_desktop);
    setImageBase64Mobile(banner.banner_image_mobile);
  }, [banner]);
  ...
  async function onSubmit(values: UpdateBannerFormType) {
    setLoading(true);
    const banner_image_desktop =
      typeof values.banner_image_desktop === "string"
        ? values.banner_image_desktop
        : {
    ...
    const banner_image_mobile =
      typeof values.banner_image_mobile === "string"
    ...
    axios
      .put(
        `${process.env.ENDPOINT}/api/banners/${id}`,
        {
          banner_image_desktop,
          banner_image_mobile,
          link: values.link,
