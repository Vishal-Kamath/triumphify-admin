...
} from "./create-attribute-form-schema";
...
import { invalidateAllAttributes } from "@/lib/attributes";
import { v4 as uuid } from "uuid";

const CreateAttributeForm: FC = () => {
  ...
  async function onSubmit(values: CreateAttributeType) {
    if (!!tempValue.trim()) {
      values.values.push({
        id: uuid(),
        name: tempValue.trim(),
    ...
    setLoading(true);
    axios
      .post(`${process.env.ENDPOINT}/api/attributes/`, values, {
        headers: {
              ...
              <div className="flex flex-wrap gap-3">
                {form.watch("values").map((value, index) => (
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={(e) => removeValue(value.id)}
