...
const UpdateUsername: FC<{ employee_username: string }> = ({
  employee_username,
}) => {
  ...
  useEffect(() => {
    if (!!employee_username) {
      form.setValue("username", employee_username);
    }
  }, [employee_username]);
  ...
  function onSubmit(values: UsernameType) {
    setLoading(true);
    axios
      .patch(`${process.env.ENDPOINT}/api/employees/update/username`, values, {
    ...
    <Form {...form}>
      ...
      >
        ...
        {loading ? (
          ...
          <span className="text-sm text-slate-500">{employee_username}</span>
        )}
      </form>
    </Form>
  );
